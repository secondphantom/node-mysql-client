...
exports.DbClient = exports.QueryBuilder = void 0;
    ...
    static findBuilder(args, params = undefined) {
        ...
        if (include) {
            for (const args of Object.values(include)) {
                QueryBuilder.findBuilder(args, { ...params, parentTableName: tableName });
            }
        }
        if (take) {
...
class DbClient {
    ...
    dbSchemaAry;
    presentDbSchema = {};
    constructor(dbSchemaAry, dbSchemaPath, poolConfig) {
        ...
        for (const dbSchemaData of this.dbSchemaAry) {
            ...
            for await (const [dbSchemaName, dbSchemaConfig] of dbSchema) {
                ...
                if (!isDiffSchema)
                    continue;
                try {
                    const dropForeignKeyStrAry = QueryBuilder.dropForeignKey(dbSchemaConfig);
            ...
            for (const [dbSchemaName, dbSchemaConfig] of dbSchema) {
                ...
                if (!isDiffSchema)
                    continue;
                const createForeignKeyQueryStrAry = QueryBuilder.createForeignKey(dbSchemaConfig);
